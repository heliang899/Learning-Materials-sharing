...
![KMPè¯¦è§£1](https://camo.githubusercontent.com/efed0a1e96c2f77182f3beb78cb01b40867e7c75d11e6957e1f67d21c39dd7de/68747470733a2f2f636f64652d7468696e6b696e672e63646e2e626365626f732e636f6d2f676966732f4b4d50254537254232254245254538254145254232312e676966)
...
public: 
    void getnext(int *next,const string &s){
        ...
        next[0] = j;// 此行容易忘记
        for (int i = 1; i < n; i++){
            //前后缀不同  如果 s[i] 与 s[j + 1] 不相同，j就要从next数组里寻找下一个匹配的位置。
            while (j >= 0 && s[ i ] != s[j + 1]){
                j = next[j];//回退
            }
            //找到相同前后缀
...
#### 7 [5. 最长回文子串](https://leetcode-cn.com/problems/longest-palindromic-substring/)
...
```c++
//最好情况，o(n) ,最坏情况 o(n^2) ,稳定排序 
void bubble_sort(vector<int> &v){
	for(int i=0;i<v.size();i++){
		bool flag=false;//标记，优化 
...
## 1)[93. 复原 IP 地址](https://leetcode-cn.com/problems/restore-ip-addresses/)
...
class singleton {
    ...
    public:
    	static singleton* getInstance(){    
            if ( instance == nullptr ) {  //双检查锁实现线程安全
                Lock();
                if ( instance == nullptr ) {
                    instance = new singleton();
                }
...
int main() {
    v_struct* p = (v_struct*) malloc( sizeof(v_struct) + sizeof(int) * 100 ); //长度100；
    p->a[100] = 100;
}
```
